...
package net.mcreator.oaksnature.world.features;
...
public class DigSiteAFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public DigSiteAFeature() {
		super(NoneFeatureConfiguration.CODEC);
		base_blocks = List.of(Blocks.STONE, Blocks.GRASS_BLOCK, Blocks.MYCELIUM, Blocks.DIRT, Blocks.SAND, Blocks.RED_SAND, Blocks.GRAVEL);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("oaks_nature", "arc_site_a"));
		if (template == null)
			return false;
