...
public class GiantPumpkinFeature extends Feature<NoneFeatureConfiguration> {
	...
	public GiantPumpkinFeature() {
		...
		base_blocks = List.of(Blocks.GRASS_BLOCK, Blocks.DIRT, Blocks.COARSE_DIRT, Blocks.PODZOL, Blocks.ROOTED_DIRT);
	}
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("oaks_nature", "graint_pumpkin_2"));
		if (template == null)
			return false;
